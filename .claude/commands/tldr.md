...
```markdown
- [Post Title](/tldr/filename) - Short description
```
...
- Examples: `cloudflare-fuse-containers.md`, `agent-harnesses.md`
...
For a post about "New React Server Components":

```markdown
---
title: "TL;DR: React Server Components"
description: Server-side React components that render on server, send HTML to client.
---

React components that never ship JS to the browser - they run on the server only.

```
┌─────────────────────────────────────────────────────┐
│                    REQUEST FLOW                      │
│                                                      │
│  Browser ──► Server Component ──► HTML + Client JS   │
│     │              │                    │            │
│     │              ▼                    │            │
│     │        DB / API calls             ▼            │
│     │        (no client fetch)     Interactive UI    │
│     │                                   │            │
│     ◄───────────────────────────────────┘            │
└─────────────────────────────────────────────────────┘
```
...
## Key Benefits
- **Zero client JS**: Server components don't add to bundle size
- **Direct data access**: Query DB/APIs without API routes
- **Streaming**: Send HTML as it renders, no waterfall

## Trade-offs
- **No interactivity**: Need client components for onClick, useState
- **Server dependency**: Every render needs server round-trip

## Reference
- [React Server Components](https://react.dev/...) - React Docs
```
