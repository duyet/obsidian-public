...
I picked Claude Code SDK because it seemed powerful for multi-agent orchestration. I would only need to define the sub-agents or skills for each capability like code review, checking PRs, news summary, etc. There is something I wonder about that is not clear, like how sessions, memory, etc. are organized.
...
I opened plan mode again and asked for an update to the design:
...
- Which LLM provider to use? I chose **OpenRouter** since it has a free tier and supports Grok models. I am have a custom note for using OpenRouter via Cloudflare AI Gateway which only need to binding directly to the workers.

I opened Cloudflare Dashboard to configure an AI Gateway, named it "duyetbot" and added the OpenRouter key. Using grok-4.1-fast model which is still free and quite promising, fan of Grok now. This project inspired when people can tag @grok on X for any question and it is perfectly answered anything.
...
I started asking Claude Code to implement. This took a long time. Claude Code Web currently offers $1000 for free usage during beta testing.
...
*[Next: Day 2 - Redesign](./day-2-redesign)*
